...
The =jref= accessor function is an interpretation of the JSON Pointer
standard (see RFC 6901) with a Lisp like interface.  It operates on
the Lisp data structure of a JSON value.  For example,
#+BEGIN_SRC common-lisp
(let ((data (parse "[{\"foo\" : 42, \"bar\" : [\"baz\", \"hack\"]}, null]")))
  (setf (jref data 0 "bar" 1) :true)
  (serialize t (jref data 0 "bar")))
#+END_SRC
prints
#+BEGIN_EXAMPLE
["baz", true]
...
- RS-JSON has a nice balance between speed and memory usage.  For a
  small speed penalty, you can optimize memory usage by setting the
  =*string-as-base-string*= special variable to true.
